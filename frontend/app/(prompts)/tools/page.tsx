...
export default function ToolsPage() {
  ...
  const handleCreateNew = () => {
    if (selectedRepository) {
      router.push(`/tools/new?repo=${encodeURIComponent(selectedRepository)}`);
    }
  };

  const handleEdit = (toolName: string) => {
    if (selectedRepository) {
      router.push(`/tools/${encodeURIComponent(toolName)}?repo=${encodeURIComponent(selectedRepository)}`);
    }
  };
