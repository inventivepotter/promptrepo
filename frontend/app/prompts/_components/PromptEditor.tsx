...
export function PromptEditor() {
  ...
  const isNewPrompt = pathname.includes('/new');
  ...
  const handleSave = async () => {
    ...
    toaster.promise(savePromise, {
      ...
      success: (data) => {
        ...
        // If we're creating a new prompt, redirect to the actual prompt URL
        if (isNewPrompt && data?.file_path && data?.repo_name) {
          setTimeout(() => {
            router.push(buildEditorUrl(data.repo_name, data.file_path));
          }, 100);
        }
