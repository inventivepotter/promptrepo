...
import type { MetricConfig, MetricType } from '@/types/test';
...
export function MetricSelector({ metrics, onChange }: MetricSelectorProps) {
  ...
  const handleUpdateMetric = (index: number, field: keyof MetricConfig, value: unknown) => {
      ...
      {metrics.length === 0 ? (
        ...
        >
          <Text fontSize="sm" color="gray.600">
            No metrics configured. Add metrics below to start testing.
          </Text>
        </Box>
      ) : (
        <VStack gap={3} align="stretch">
          {metrics.map((metric, index) => {
            ...
            const currentModelValue = metric.model || '';
        ...
        <VStack align="stretch" gap={4}>
          ...
          <Field.Root>
            <Combobox.Root
              ...
              inputValue={modelSearch || availableMetrics.find((m) => m.value === selectedMetricType)?.label || ''}
              onInputValueChange={(e) => setModelSearch(e.inputValue)}
              openOnClick
              size="sm"
              width="full"
            ...
            <Field.HelperText fontSize="xs">
              Select a metric to configure for this test suite
            </Field.HelperText>
          </Field.Root>
