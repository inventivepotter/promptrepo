...
from services.git_provider.git_provider_factory import OAuthProviderFactory
...
from services.git_provider.providers.gitlab_provider import GitLabOAuthProvider
from services.git_provider.providers.bitbucket_provider import BitbucketOAuthProvider
...
class TestOAuthService:
    ...
    @pytest.fixture
    def git_provider_service(self, mock_config_service):
        ...
        service = GitProviderService(mock_config_service, auto_register=False)
        return service
    ...
    @pytest.mark.asyncio
    async def test_get_authorization_url_success(self, git_provider_service, redirect_uri, scopes):
    ...
    async def test_get_authorization_url_validation(self, git_provider_service, provider, redirect_uri, expected_error, exception_type):
    ...
    async def test_exchange_code_for_token_success(self, git_provider_service, auth_code, redirect_uri, test_state, github_token_response):
        ...
        # Store state first
        git_provider_service.state_manager.store_state(
            state=test_state,
            provider="github",
            redirect_uri=redirect_uri
        )
    ...
    @pytest.mark.asyncio
    async def test_exchange_code_for_token_invalid_state(self, git_provider_service, auth_code, redirect_uri):
    ...
    @pytest.mark.asyncio
    async def test_exchange_code_for_token_redirect_uri_mismatch(self, git_provider_service, auth_code, test_state):
    ...
    def test_cleanup_expired_states(self, git_provider_service, sample_oauth_state, expired_oauth_state):
        ...
        git_provider_service.state_manager._states[expired_oauth_state.state] = expired_oauth_state
        
        # Verify both states exist
        assert len(git_provider_service.state_manager._states) == 2
        
        # Clean up expired states
        removed_count = git_provider_service.cleanup_expired_states()
        assert removed_count == 1
        assert sample_oauth_state.state in git_provider_service.state_manager._states
        assert expired_oauth_state.state not in git_provider_service.state_manager._states

    def test_get_provider_config(self, git_provider_service):
        """Test getting provider configuration."""
        # Test GitHub provider
        config = git_provider_service.get_provider_config("github")
        ...
        config = git_provider_service.get_provider_config("unknown")
