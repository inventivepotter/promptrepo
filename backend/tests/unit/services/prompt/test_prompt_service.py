...
@pytest.fixture
def prompt_service(mock_config_service, mock_file_ops_service):
...
class TestUpdatePrompt:
    ...
    async def test_update_prompt_success(self, prompt_service, mock_file_ops_service):
        ...
        update_data = PromptDataUpdate(name="New Name", prompt="New content")
            ...
            with patch.object(prompt_service, '_get_file_commit_history', return_value=[]):
                with patch.object(prompt_service, 'get_prompt') as mock_get:
                    ...
                    mock_get.side_effect = [
                        PromptMeta(
                            prompt=PromptData(**existing_data),
                            repo_name=repo_name,
                            file_path=file_path
                        ),
                        PromptMeta(
