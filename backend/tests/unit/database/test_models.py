...
class TestUserModel:
    ...
    def test_user_creation(self, db_session: Session):
        ...
        assert saved_user.oauth_provider == "github"
    ...
    def test_user_creation_minimal_fields(self, db_session: Session):
        ...
        user = User(oauth_username="minimal_user", oauth_provider="github")
        db_session.add(user)
        db_session.commit()
    ...
    def test_user_different_oauth_providers(self, db_session: Session):
        ...
        user1 = User(oauth_username="user1", oauth_provider="github", oauth_user_id=123)
        user2 = User(oauth_username="user2", oauth_provider="gitlab", oauth_user_id=123)
    ...
    def test_user_timestamps(self, db_session: Session):
        ...
        user = User(oauth_username="timestamp_user", oauth_provider="github", oauth_user_id=789)
        db_session.add(user)
        db_session.commit()
    ...
    def test_user_update_timestamp(self, db_session: Session):
        ...
        user = User(oauth_username="update_user", oauth_provider="github", oauth_user_id=999)
        db_session.add(user)
        db_session.commit()
    ...
    def test_user_string_representation(self, db_session: Session):
        ...
        user = User(oauth_username="str_user", oauth_provider="github", oauth_user_id=111)
        db_session.add(user)
        db_session.commit()
    ...
    def test_user_query_by_oauth_provider(self, db_session: Session):
        ...
        github_user = User(oauth_username="github_user", oauth_provider="github", oauth_user_id=1)
        gitlab_user = User(oauth_username="gitlab_user", oauth_provider="gitlab", oauth_user_id=2)
        github_user2 = User(oauth_username="github_user2", oauth_provider="github", oauth_user_id=3)
        ...
        # Query by provider
        github_users = db_session.exec(select(User).where(User.oauth_provider == "github")).all()
        gitlab_users = db_session.exec(select(User).where(User.oauth_provider == "gitlab")).all()

        assert len(github_users) == 2
        assert len(gitlab_users) == 1
        assert all(user.oauth_provider == "github" for user in github_users)
        assert all(user.oauth_provider == "gitlab" for user in gitlab_users)
...
class TestUserSessionModel:
    ...
    def test_session_creation(self, db_session: Session):
        ...
        # First create a user
        user = User(oauth_username="session_user", oauth_provider="github", oauth_user_id=101)
        db_session.add(user)
        db_session.commit()
    ...
    def test_session_update_accessed_at(self, db_session: Session):
        ...
        # Create user
        user = User(oauth_username="access_user", oauth_provider="github", oauth_user_id=101)
        db_session.add(user)
        db_session.commit()
    ...
    def test_session_string_representation(self, db_session: Session):
        ...
        # Create user
        user = User(oauth_username="str_session_user", oauth_provider="github", oauth_user_id=102)
        db_session.add(user)
        db_session.commit()
    ...
    def test_session_multiple_sessions_per_user(self, db_session: Session):
        ...
        # Create user
        user = User(oauth_username="multi_session_user", oauth_provider="github", oauth_user_id=103)
        db_session.add(user)
        db_session.commit()
...
class TestUserReposModel:
    ...
    def test_repository_creation(self, db_session: Session):
        ...
        # First create a user
        user = User(oauth_username="repo_user", oauth_provider="github", oauth_user_id=201)
        db_session.add(user)
        db_session.commit()
    ...
    def test_repository_creation_with_minimal_fields(self, db_session: Session):
        ...
        # First create a user
        user = User(oauth_username="minimal_repo_user", oauth_provider="github", oauth_user_id=456)
        db_session.add(user)
        db_session.commit()
    ...
    def test_repository_multiple_repos_per_user(self, db_session: Session):
        ...
        # First create a user
        user = User(oauth_username="multi_repo_user", oauth_provider="github", oauth_user_id=301)
        db_session.add(user)
        db_session.commit()
    ...
    def test_repository_same_name_different_users(self, db_session: Session):
        ...
        # Create two users
        user1 = User(oauth_username="user1", oauth_provider="github", oauth_user_id=401)
        user2 = User(oauth_username="user2", oauth_provider="github", oauth_user_id=402)
        db_session.add(user1)
        db_session.add(user2)
        db_session.commit()
    ...
    def test_repository_unique_constraint(self, db_session: Session):
        ...
        # Create user
        user = User(oauth_username="unique_repo_user", oauth_provider="github", oauth_user_id=111)
        db_session.add(user)
        db_session.commit()
    ...
    def test_repository_query_by_status(self, db_session: Session):
        ...
        # First create a user
        user = User(oauth_username="status_repo_user", oauth_provider="github", oauth_user_id=501)
        db_session.add(user)
        db_session.commit()
    ...
    def test_repository_string_representation(self, db_session: Session):
        ...
        # Create user
        user = User(oauth_username="str_repo_user", oauth_provider="github", oauth_user_id=444)
        db_session.add(user)
        db_session.commit()
        ...
        assert str(repo.status) in str_repr