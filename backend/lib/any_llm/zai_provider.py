"""
Z.AI provider implementation following AnyLLM standard pattern.
...
class ZAIProvider(AnyLLM):
    """
    Z.AI Provider following AnyLLM standard pattern.
    
    Uses httpx client to communicate with Z.AI API server.
    Read more here - https://www.z.ai/
    """

    PROVIDER_NAME = "zai"
    PROVIDER_DOCUMENTATION_URL = "https://www.z.ai/"
    ...
    def _convert_completion_params(params: CompletionParams, **kwargs: Any) -> dict[str, Any]:
        """Convert CompletionParams to kwargs for Z.AI API."""
    ...
    def _convert_completion_response(response: dict[str, Any]) -> ChatCompletion:
        """Convert Z.AI response to OpenAI format ChatCompletion."""
        from any_llm.types.completion import (
            ChatCompletion,
        ...
        for choice_data in response.get("choices", []):
            ...
            # ZAI uses 'reasoning_content' for reasoning models (e.g., GLM-4.6)
        ...
        return ChatCompletion(
            id=response.get("id", "zai-completion"),
    ...
    def _convert_completion_chunk_response(response: dict[str, Any], **kwargs: Any) -> ChatCompletionChunk:
        """Convert Z.AI chunk response to OpenAI format ChatCompletionChunk."""
        from any_llm.types.completion import (
            ChatCompletionChunk,
            ChoiceDelta,
            ChunkChoice,
        ...
        for choice_data in response.get("choices", []):
            ...
            # ZAI uses 'reasoning_content' for reasoning models
            # If content is empty but reasoning_content exists, use reasoning_content
            content = delta_data.get("content", "")
            if not content and "reasoning_content" in delta_data:
                content = delta_data.get("reasoning_content", "")
    ...
    def _convert_embedding_params(params: Any, **kwargs: Any) -> dict[str, Any]:
        """Convert embedding parameters for Z.AI (not supported)."""
        raise NotImplementedError("Z.AI does not support embeddings")

    @staticmethod
    def _convert_embedding_response(response: dict[str, Any]) -> CreateEmbeddingResponse:
        """Convert Z.AI embedding response to OpenAI format (not supported)."""
        raise NotImplementedError("Z.AI does not support embeddings")

    @staticmethod
    def _convert_list_models_response(response: Any) -> Sequence[Model]:
        """Convert Z.AI list models response to OpenAI format."""
        from any_llm.types.model import Model

        # Z.AI has predefined models
        predefined_models = ["glm-4.6", "glm-4.5", "glm-4.5-air"]
        return [
            Model(id=model_id, object="model", created=0, owned_by="zai")
            for model_id in predefined_models
        ]

    def _init_client(self, api_key: str | None = None, api_base: str | None = None, **kwargs: Any) -> None:
        """Initialize httpx async client for Z.AI API."""
        if not api_base:
    ...
    def _verify_and_set_api_key(self, api_key: str | None = None) -> str | None:
        ...
        if not api_key:
            raise ValueError("API key is required for Z.AI provider")
        return api_key
    ...
    async def _stream_completion_async(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise Exception(f"Z.AI streaming API error: {e.response.status_code}")
    ...
    async def _acompletion(
        ...
        **kwargs: Any,
        ...
        """Create a chat completion using Z.AI."""
    ...
    async def _aembedding(
        ...
        **kwargs: Any,
        ...
        """Generate embeddings using Z.AI (not supported)."""
        raise NotImplementedError("Z.AI does not support embeddings")
